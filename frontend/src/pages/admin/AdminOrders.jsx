...
const AdminOrders = () => {
    ...
    useEffect(() => {
        fetchAdvancedStats();
    }, [dateFrom, dateTo, filterStatus, filterPayment, userInfo?.token]);
    ...
    const statusOptions = statusOptionsBase.map(s => ({ ...s, label: t(`admin.status.${s.value}`) || s.value }));
    const getStatusDetails = (value) => statusOptions.find(s => s.value === value) || statusOptions[0];
    ...
    return (
        <div className="p-8">
            <div className="mb-8 text-left">
                <h1 className="text-2xl font-bold text-gray-800">{t('admin.orders.title') || 'Quản lý Đơn Bán Hàng'}</h1>
                <p className="text-gray-500 text-sm mt-1">{t('admin.orders.total', { count: orders?.length }) || `Tổng số đơn: ${orders?.length || 0}`}</p>
                
                {advStats && (
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 my-4">
                        {[
                            { label: 'Tổng đơn', value: advStats?.totalOrders || 0, color: 'bg-blue-50 text-blue-700' },
                            { label: 'Tổng doanh thu', value: `${(advStats?.totalRevenue || 0).toLocaleString('vi-VN')} VNĐ`, color: 'bg-green-50 text-green-700' },
                            { label: 'Đã thanh toán', value: `${(advStats?.revenueByStatus?.paid || 0).toLocaleString('vi-VN')} VNĐ`, color: 'bg-purple-50 text-purple-700' },
                            { label: 'Đã hủy', value: `${(advStats?.revenueByStatus?.cancelled || 0).toLocaleString('vi-VN')} VNĐ`, color: 'bg-red-50 text-red-700' },
                        ].map((s, i) => (
                            <div key={i} className={`rounded-2xl p-4 ${s.color}`}>
            ...
            <AdminFilter
                search={search}
                onSearch={setSearch}
                searchPlaceholder="Tìm theo mã đơn hoặc tên khách..."
                dateFrom={dateFrom}
                dateTo={dateTo}
                onDateFrom={setDateFrom}
                onDateTo={setDateTo}
                        ...
                        return (
                            <div key={order._id} className="bg-white rounded-2xl border border-gray-100 overflow-hidden shadow-sm">
                                ...
                                >
                                    <div className="grid grid-cols-5 gap-4 text-left flex-1 items-center">
                                        <div>
                                            <p className="text-[10px] uppercase font-bold text-gray-400 mb-1">Mã đơn</p>
                                            <p className="font-mono text-sm font-medium text-gray-700">#{order._id?.slice(-8).toUpperCase()}</p>
                                        </div>
                                        <div>
                                            <p className="text-[10px] uppercase font-bold text-gray-400 mb-1">Khách hàng</p>
                                            <p className="text-sm font-medium text-gray-700 truncate">{order.user?.name || 'Khách vãng lai'}</p>
                                        </div>
                                        <div>
                                            <p className="text-[10px] uppercase font-bold text-gray-400 mb-1">Tổng tiền</p>
                                            <p className="text-sm font-bold text-emerald-600">{(order.totalPrice || 0).toLocaleString('vi-VN')} VNĐ</p>
                                        </div>
                                        <div>
                                            <p className="text-[10px] uppercase font-bold text-gray-400 mb-1">Ngày tạo</p>
                                ...
                                {isExpanded && (
                                    <div className="border-t border-gray-100 bg-gray-50/50 p-6">
                                        <div className="grid md:grid-cols-2 gap-8">
                                            <div className="space-y-4">
                                                <h3 className="text-xs font-bold text-gray-400 uppercase tracking-widest">Sản phẩm trong đơn</h3>
                                                {(order.orderItems || order.items || []).map((item, i) => (
                                                    <div key={i} className="flex items-center gap-4 bg-white p-3 rounded-xl border border-gray-100">
                                                        <img
                                                            src={item.image || 'https://placehold.co/60x60/f3f4f6/9ca3af?text=GL'}
                                                            alt={item.name}
                                                            className="w-16 h-16 object-cover rounded-lg"
                                                        />
                                                        <div className="flex-1 text-left">
                                                            <p className="text-sm font-semibold text-gray-800">{item.name}</p>
                                                            <p className="text-xs text-gray-500">Số lượng: {item.quantity} x {(item.price || 0).toLocaleString('vi-VN')} VNĐ</p>
                                                        </div>
                                                    </div>
                                                ))}
                                            </div>
                                            ...
                                            <div className="text-left space-y-6">
                                                <div>
                                                    <h3 className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-3">Địa chỉ giao hàng</h3>
                                                    <div className="text-sm text-gray-600 bg-white p-4 rounded-xl border border-gray-100 space-y-1">
                                                        <p className="font-bold text-gray-800">{order.shippingAddress?.fullName || order.shippingAddress?.fullname}</p>
                                                        <p>{order.shippingAddress?.phone}</p>
                                                        <p>{order.shippingAddress?.address}</p>
                                                ...
                                                <div>
                                                    <h3 className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-3">Cập nhật trạng thái</h3>
                                                    <div className="relative max-w-[200px]">
                                                        <select
                                                            value={order.orderStatus}
                                                            onChange={e => handleStatusChange(order._id, e.target.value)}
