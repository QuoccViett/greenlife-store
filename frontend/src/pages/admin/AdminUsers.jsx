...
const AdminUsers = () => {
    ...
    useEffect(() => {
        const fetchUsers = async () => {
            ...
            try {
                const { data } = await axios.get(`${API}/admin/users`, config)
                // Ép dữ liệu về Mảng an toàn tránh lỗi users.filter is not a function
    ...
    // Lọc mảng an toàn với Array.isArray
    const filteredUsers = useMemo(() => {
    ...
    return (
        <div className="p-8">
            <div className="mb-8 text-left">
                <h1 className="text-2xl font-bold text-gray-800">{t('admin.users.title') || 'Quản lý Người Dùng'}</h1>
                <p className="text-gray-500 text-sm mt-1">
                    {t('admin.users.showing', { shown: filteredUsers?.length || 0, total: users?.length || 0 }) || `Hiển thị ${filteredUsers?.length || 0} trên tổng ${users?.length || 0}`}
                </p>
            </div>
            ...
            <AdminFilter
                search={search}
                onSearch={setSearch}
                searchPlaceholder={t('admin.users.search_placeholder') || 'Tìm kiếm người dùng...'}
                dropdowns={[
                    {
                        value: filterRole,
                        onChange: setFilterRole,
            ...
            <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden shadow-sm mt-4">
                {loading ? (
                    ...
                    <div className="p-20 text-center text-gray-400">
                        {t('admin.users.none', { query: search }) || 'Không tìm thấy người dùng phù hợp'}
                    </div>
                ) : (
                    <div className="overflow-x-auto">
                        <table className="w-full text-sm table-fixed">
                            <thead className="bg-gray-50 border-b border-gray-100">
                                <tr>
                                    <th className="text-left px-6 py-4 text-xs font-semibold text-gray-500 uppercase tracking-wide">{t('admin.users.table.user') || 'Người dùng'}</th>
                                    <th className="px-6 py-4 text-xs font-semibold text-gray-500 uppercase tracking-wide">{t('admin.users.table.contact') || 'Liên hệ'}</th>
                                    <th className="px-6 py-4 text-xs font-semibold text-gray-500 uppercase tracking-wide">{t('admin.users.table.joined') || 'Ngày tham gia'}</th>
                                    <th className="text-right px-6 py-4 text-xs font-semibold text-gray-500 uppercase tracking-wide">{t('admin.users.table.access_lv') || 'Quyền'}</th>
                                    <th className="text-left px-6 py-4 text-xs font-semibold text-gray-500 uppercase tracking-wide">{t('admin.users.table.action') || 'Thao tác'}</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-gray-50">
                                {filteredUsers.map(user => (
                                    <tr key={user._id} className="hover:bg-gray-50/50 transition">
                                        ...
                                        <td className="px-6 py-4">
                                            <div className="flex flex-col">
                                                <span className="text-gray-700">{user.email}</span>
                                                <span className="text-gray-400 text-xs">{user.phone || t('admin.users.no_phone') || 'Chưa có SĐT'}</span>
                                            </div>
                                        </td>
                                        <td className="px-6 py-4 text-gray-400 text-xs">
                                        ...
                                        <td className="px-6 py-4 text-right">
                                            {user._id === userInfo?._id ? (
                                                <span className="inline-flex items-center gap-1.5 bg-green-100 text-green-700 text-xs font-bold px-3 py-1.5 rounded-full uppercase tracking-tighter">
                                                    <IconShield className="w-3.5 h-3.5" />
                                                    {t('admin.users.current_admin') || 'ADMIN HIỆN TẠI'}
                                                </span>
                                            ) : (
                                                <div className="relative inline-block text-left">
                                                    <select
